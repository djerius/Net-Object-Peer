...
The proxy object identifies itself as the upstream emitter when it
sends the event.

=item *

The object subscribes to the C<detach> event in addition to the one it
will translate, so it can, in turn, send a C<detach> event to it's
listeners.  A good listener will then unsubscribe.

=back

and here's code to use it:
...
The drawback with this approach is that the listener seemingly
recieves an event from an object (C<$nB>) which it hasn't subscribed
to.  This can cause a problem for introspection, as the listener's list
of subscriptions will not include C<$nB>.

An alternative is for C<TranslageBtoA> to act as a pass through for
all communications with C<$nB>.  
